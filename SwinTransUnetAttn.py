...
def window_partition(x,window_size):
    B,H,W,C=x.shape
    x=x.view(B,H//window_size,W//window_size,window_size,C)
...
class SwinTransformerBlock(nn.Module):
    ...
    qkv_bias(bool,optional): If True, add a learnable bias to query,key,value. Default True.
    qk_scale(float| Optional): Ovveride default qk scale of head_dim ** -0.5 if set.
    drop(float,optional): Dropout rate. Default 0.0
    attn_drop(float,optional): weight droppinf of attention. Default : 0.0 
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim},input_resolution={self.input_resolution},num_heads={self.num_heds},"\
...
class PatchMerging(nn.Module): 
    ...
    Args: 
    input_resolution(tuple[int]): Resolution of input features. 
    dim (int): No of input channels 
...
class SwinTransformerSys(nn.Module):
    def __init__(self,img_size=224,patch_size=4,in_chans=3,num_classes=1000,
        ...
        print(
            "SwinTransformerSys expand initial---depths:{};depths_decoder:{};drop_path_rate:{};num_classes:{}".format(depths,depths_decoder,drop_path_rate,num_classes)
        )
        ...
        self.patches_resolution = patches_resolution
        # our method of introducing the Attention 
        self.mha = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=num_heads[0], batch_first=True)
    ...
    def forward_up_features(self,x,x_downsample):
        for inx,layer_up in enumerate(self.layers_up):
            if inx==0:
                ...
                skip=skip.view(B,H,W,C).permute(0,3,1,2)

                # combine features using MHA 
                x=x.flatten(2).permute(0,2,1)
                skip=skip.flatten(2).permute(0,2,1)
                x,_=self.mha(x,skip,skip)

                # reshape Back 
                x=x.permute(0,2,1).view(B,C,H,W).permute(0,2,3,1)
                x=x.flatten(1,2)
    ...
    def flops(self):
        ...
        for i,layers in enumerate(self.layers):
            flops+=layers.flops()
        flops+=self.num_features*self.patches_resolution[0]*self.patches_resolution[1]//2(2**self.num_layers)
